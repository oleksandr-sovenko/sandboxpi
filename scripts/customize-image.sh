...
Main() {
	case $RELEASE in
		...
		bionic|focal)
			...
			# WiringOP-Zero
			git clone https://github.com/oleksandr-sovenko/WiringOP-Zero /tmp/WiringOP-Zero
			...
			old_overlays=$(cat /boot/armbianEnv.txt | grep overlays= | sed 's@overlays=@@')
			new_overlays=(${old_overlays} analog-codec cir i2c0 i2c1 i2c2 pps-gpio pwm spdif-out spi-add-cs1 spi-jedec-nor spi-spidev uart1 uart2 uart3 usbhost0 usbhost1 usbhost2 usbhost3 w1-gpio)
			new_overlays=($(echo "${new_overlays[@]}" | tr ' ' '\n' | sort -u | tr '\n' ' '))
			sed -i "s@`echo ${old_overlays}`@`echo ${new_overlays[@]}`@" /boot/armbianEnv.txt
